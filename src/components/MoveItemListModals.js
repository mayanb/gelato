...
class MoveItemListModalUnconnected extends Component {
	...
	renderRow({item, index}) {
		const process = this.props.processHash[item.creating_task.process_type]
		const processIconPath = process ? process.icon : ''
		return <QRItemListRow
			qr={item['item_qr']}
			task_display={item.creating_task.display}
			imgpath={processIconPath}
			onRemove={() => this.props.onRemove(index)}
			onOpenTask={() => this.props.onOpenTask(item.creating_task)}
		/>
	}
...
export const MoveItemListModal = connect(mapStateToProps)(MoveItemListModalUnconnected)

function inputHeader(items, typeName, restOfName, unit) {
	...
	return (
		<View style={styles.container}>
			<Text style={styles.text}>
				{`${count} ${pluralize(typeName, count)} ${restOfName}`}
			</Text>
		</View>
	)
}
